...
const generateAccessandRefereshToken = async (userId) => {
    try {
      ...
      return {accessToken,refereshToken}
        ...
        throw new ApiError(500,"somthing went worng when generating referesh or access token")
    }
}
...
const loginUser = asyncHandler(async (req, res)=>{
   ...
   const {refereshToken,accessToken} =await generateAccessandRefereshToken(user._id)
    ...
    return res.status(200).cookie("accessToken",accessToken,options).cookie("refreshToken",refereshToken,options)
    .json(
        new ApiResponse(
            200,{
